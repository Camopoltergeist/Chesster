...
pub fn get_collision_mask(board: Board, column: u32, rank: u32) -> Bitboard {
    ...
    match piece {
        ...
        Piece::Rook => return get_rook_collision(board, player, Bitboard::coordinates_to_bit_offset(column, rank)),
...
pub fn get_rook_collision(board: Board, player: Player, offset: u32) -> Bitboard {
    unsafe {
        let mut valid_moves: u64 = ROOK_MASKS[offset as usize].into();
        let mut collision_mask: u64 = ((board.white_pieces | board.black_pieces) & valid_moves).into();

    if collision_mask != 0 {
        let rank_mask: u64 = 0xFE;
        let column_mask: u64 = 0x101010101010100;

        let n_collision = (column_mask << offset) & collision_mask;
        if n_collision != 0 {
            let n_offset = n_collision.trailing_zeros();
            if board.get_player_bitboard(player.opposite()).check_bit(n_offset) {
        ...
        if w_collision != 0 {
            ...
            if board.get_player_bitboard(player.opposite()).check_bit(w_offset) {
                valid_moves &= !((rank_mask >> w_offset % 8) << (w_offset + 1));
            } else {
                valid_moves &= !(rank_mask >> w_offset % 8); //Check this later
            }
            collision_mask &= !w_collision;
        }

        let s_collision = column_mask >> (64 - offset) & collision_mask;
        if s_collision != 0 {
            let s_offset =  63 - s_collision.leading_zeros();
            if board.get_player_bitboard(player.opposite()).check_bit(s_offset) {
                valid_moves &= !(get_column_mask(s_offset % 8, s_offset / 8));
            } else {
                valid_moves &= !(get_column_mask(s_offset % 8, s_offset / 8 + 1));
                println!("{:?} {}", {}, get_column_mask(s_offset % 8, s_offset / 8));
        ...
        if collision_mask != 0 {
            ...
            if board.get_player_bitboard(player.opposite()).check_bit(e_offset) {
                valid_moves &= !((rank_mask >> 8 - (e_offset % 8)) << (e_offset - (e_offset % 8)));
            } else {
                valid_moves &= !((rank_mask >> 7 - (e_offset % 8)) << (e_offset - (e_offset % 8)));
            }
        }
    }
